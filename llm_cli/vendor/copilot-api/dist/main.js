...
//#endregion
//#region src/lib/enterprise.ts
const ENTERPRISE_URL_ENV = "COPILOT_ENTERPRISE_URL";
const enterpriseArgs = { "enterprise-url": {
	type: "string",
	description: `GitHub Enterprise tenant (e.g. mycompany.ghe.com). Can also be set via ${ENTERPRISE_URL_ENV}`
} };
function normalizeDomain(value) {
	return (value ?? "").replace(/^https?:\/\//, "").replace(/\/+$/, "") || void 0;
}
function applyEnterpriseDomain(value) {
	state.enterpriseDomain = normalizeDomain(value ?? process.env[ENTERPRISE_URL_ENV]);
...
function githubTokenPath(domain) {
...
const copilotBaseUrl = (state$1) => {
	if (state$1.enterpriseDomain) return `https://copilot-api.${state$1.enterpriseDomain}`;
...
const githubApiBaseUrl = (state$1) => state$1.enterpriseDomain ? `https://api.${state$1.enterpriseDomain}` : "https://api.github.com";
...
const githubBaseUrl = (state$1) => state$1.enterpriseDomain ? `https://${state$1.enterpriseDomain}` : "https://github.com";
...
const readGithubToken = () => fs.readFile(githubTokenPath(state.enterpriseDomain), "utf8");
const writeGithubToken = (token) => fs.writeFile(githubTokenPath(state.enterpriseDomain), token);
...
async function runAuth(options) {
	...
	if (enterpriseDomain) consola.info(`Using GitHub Enterprise tenant ${enterpriseDomain}`);
	await ensurePaths();
...
const checkUsage = defineCommand({
	...
	args: { ...enterpriseArgs },
	async run({ args }) {
		applyEnterpriseDomain(args["enterprise-url"]);
		await ensurePaths();
...
async function checkTokenExists(tokenPath) {
	try {
		if (!(await fs.stat(tokenPath)).isFile()) return false;
		return (await fs.readFile(tokenPath, "utf8")).trim().length > 0;
	} catch {
		return false;
	}
}
async function getDebugInfo() {
	...
	const [version, tokenExists] = await Promise.all([getPackageVersion(), checkTokenExists(tokenPath)]);
	return {
		version,
...
const start = defineCommand({
	...
	args: {
		...
		hostname: {
			...
			description: "Address to bind to (default: loopback only — the server is unauthenticated)"
		},
		verbose: {
			alias: "v",
			type: "boolean",

#!/usr/bin/env python3
"""
Generates a compact project context index for LLM consumption.

Instead of having the LLM read every file linearly (high token cost),
this script builds a symbol index (functions, classes) grouped by directory.
The LLM reads this index first and then targets only the files it needs.

Output format (tabulaire, one line per file):
    rel/path/file.py | 120 LOC | ClassName.method1, ClassName.method2, top_level_func

Usage:
    python3 gen_context_cache.py [project_path] [output_file]

Defaults:
    project_path = current working directory
    output_file  = <tool_home>/projects/<hash>/context_cache.md
                   (tool_home = TOOL_HOME env var, else the parent of this
                   script's directory when installed, else ~/.claude)

Exclusions:
    Reads .claudeignore and .copilotignore at project root (gitignore-style
    patterns, union of both files) so one index serves both tools.
"""
...
IGNORE_FILE_NAMES = (".claudeignore", ".copilotignore")
...
def resolve_tool_home() -> Path:
    """Resolves the tool home: TOOL_HOME env, else the installed script location."""
    env_home = os.environ.get("TOOL_HOME")
    if env_home:
        return Path(env_home)
    script_parent = Path(__file__).resolve().parent.parent
    if script_parent.name in (".claude", ".copilot"):
        return script_parent
    return Path.home() / ".claude"


def get_default_output_path(project_root: Path) -> Path:
    project_hash = hashlib.md5(str(project_root).encode()).hexdigest()[:8]
    cache_dir = resolve_tool_home() / "projects" / project_hash
    cache_dir.mkdir(parents=True, exist_ok=True)
    return cache_dir / "context_cache.md"


def main():
    project_root = Path(sys.argv[1]).resolve() if len(sys.argv) > 1 else Path.cwd()
    output_file = Path(sys.argv[2]) if len(sys.argv) > 2 else get_default_output_path(project_root)
    ...
    # Check freshness before collecting files — exits immediately if nothing changed.
